# Only use for creating admin data!
import json
...
def create_admin():
    ...
    surname = input_quit_handle(green + f"Enter surname:")
    email = input_quit_handle(green + f"Enter Email:")
    phone = input_quit_handle(green + f"Enter phone:")
    ...
    encrypted_admin_data = {
        "name": sha256_encrypt(
            name
        ),  # if everything works at this to all sha256_encrypt(name)
        "surname": surname,
        "email": email,
        "phone": phone,
